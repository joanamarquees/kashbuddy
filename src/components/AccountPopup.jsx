	...
	return (
		<>
			<button
				type="button"
				onClick={openModal}
				className="mx-auto w-full h-16 items-center flex border-dashed border-[1.5px] border-zinc-500 rounded-xl active:scale-95 transition-all duration-200 cursor-pointer"
			>
								...
								<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-zinc-800 p-6 text-left align-middle shadow-xl transition-all">
									...
									<div className="px-4 pb-4 sm:p-6 sm:pb-4 flex flex-col justify-center items-center align-center gap-6">
										<div className="sm:flex sm:items-center">
											<div className="modal-box">
												...
												<Input
													...
													value={newAmount}
													onChange={(e) =>
														setNewAmount(e.target.value.replace(",", "."))
													}
												/>
											</div>
										</div>
										<div className="flex gap-2">
											...
											<Button onClick={handleUpdateAmount}>Save</Button>
										</div>
									</div>
								</Dialog.Panel>
							</Transition.Child>
